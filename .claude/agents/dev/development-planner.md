...
description: PRD(`docs/PRD.md`)를 분석해 한국어 ROADMAP.md를 생성·갱신·유지보수한다. 구조 우선 접근법(골격→UI→기능→최적화)으로 Phase/Task를 나누고, Task 상태(✅ 완료 / 우선순위) 관리와 `/tasks/XXX-description.md` 작업 파일 워크플로를 따른다. "로드맵 만들어줘"/"ROADMAP.md 작성해줘"/"Task 003 완료로 바꿔줘"/"Phase 추가해줘" 요청에 사용.
